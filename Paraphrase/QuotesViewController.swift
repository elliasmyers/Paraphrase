...
class QuotesViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView,
        ...
        guard let showQuote = storyboard?.instantiateViewController(
            withIdentifier: "ShowQuoteViewController") as? ShowQuoteViewController else {
    ...
    @objc func addQuote() {
        ...
        guard let editQuote = storyboard?.instantiateViewController(
            withIdentifier: "EditQuoteViewController") as? EditQuoteViewController else {
    ...
    override func tableView(_ tableView: UITableView,
        ...
        let edit = UITableViewRowAction(style: .normal,
                                        title: "Edit") { [unowned self] (_, indexPath) in
            let quote = self.model.quote(at: indexPath.row)
            self.selectedRow = indexPath.row

            guard let editQuote = self.storyboard?.instantiateViewController(
                withIdentifier: "EditQuoteViewController") as? EditQuoteViewController else {
                SwiftyBeaver.error("Unable to load EditQuoteViewController")
                fatalError("Unable to load EditQuoteViewController")
            }
